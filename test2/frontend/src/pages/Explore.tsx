...
export function Dashboard() {
  ...
  return <>
    ...
    <div className="overview-stats" aria-label="Статистика каталога"><div><span className="stat-icon"><FileText size={20} /></span><strong>{loading || error ? '—' : tasks.length}</strong><span>задач в каталоге</span></div><div><span className="stat-icon"><Sprout size={20} /></span><strong>{loading || error ? '—' : tasks.filter(task => task.rating.score >= 70).length}</strong><span>готовы к работе</span></div><div><span className="stat-icon"><Users size={20} /></span><strong>{loading || error || totalProposals === null ? '—' : totalProposals}</strong><span>предложений от команд</span></div></div>
...
export function Catalog() {
  ...
  return <>
    <PageHeading title="Найдите свою задачу" description="Реальные потребности бизнеса. Пространство для ваших решений." action={role === 'business' ? <Link className="button primary" to="/tasks/new"><Plus size={17} />Создать задачу</Link> : undefined} />
    <form className="catalog-filters" onSubmit={event => { event.preventDefault(); filter('topic', topicInput.trim()) }}>
      <div className="search-field"><Search size={18} /><input aria-label="Тема задачи" placeholder="Поиск по теме" list="catalog-topics" value={topicInput} maxLength={100} onChange={event => setTopicInput(event.target.value)} /><datalist id="catalog-topics">{topics.map(topic => <option value={topic} key={topic} />)}</datalist><button className="search-submit" aria-label="Применить тему" type="submit"><ArrowRight size={18} /></button></div>
      <label className="select-field"><SlidersHorizontal size={16} /><select aria-label="Уровень готовности" value={level} onChange={event => filter('level', event.target.value)}><option value="">Любая готовность</option>{Object.entries(levels).map(([value, level]) => <option key={value} value={value}>{level.label}</option>)}</select><ChevronDown size={14} /></label>
      <label className="select-field"><select aria-label="Сортировка" value={sort} onChange={event => filter('sort', event.target.value)}><option value="rating_desc">Сначала высокий рейтинг</option><option value="rating_asc">Сначала низкий рейтинг</option><option value="newest">Сначала новые</option></select><ChevronDown size={14} /></label>
    </form>
    <div className="catalog-meta"><span>{resource.loading ? 'Загружаем задачи…' : `${plural(shown.length, ['задача', 'задачи', 'задач'])}${hasMore ? ' показано' : ''}`}</span>{(topic || level) ? <button className="text-button" onClick={() => { setSearch({}); setTopicInput('') }}>Сбросить фильтры<X size={14} /></button> : <span><span className="status-dot" />Любой рейтинг — открытые возможности</span>}</div>
    {resource.loading ? <Loading cards /> : resource.error ? <ErrorNotice message={resource.error} retry={resource.retry} /> : shown.length ? <div className="task-grid">{shown.map(task => <TaskCard task={task} key={task.id} />)}</div> : <EmptyState title={topic || level ? 'По этим условиям задач пока нет' : 'Первые задачи ещё впереди'} action={topic || level ? <button className="button secondary" onClick={() => setSearch({})}>Показать все задачи</button> : role === 'business' ? <Link className="button primary" to="/tasks/new">Создать задачу</Link> : undefined}>{topic || level ? 'Попробуйте другую тему или уберите фильтр готовности.' : 'Как только бизнес опубликует задачу, вы увидите её здесь.'}</EmptyState>}
...
export function Drafts() {
  ...
  return <><PageHeading title="Рабочие задачи" description="Задачи, созданные в этом браузере. Вернитесь к ним в любой момент." action={<Link to="/tasks/new" className="button primary" onClick={() => setRole('business')}><Plus size={17} />Создать задачу</Link>} />
